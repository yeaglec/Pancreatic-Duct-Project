...
{
    // Dynamic remesh gates:
    // 1) edge too long, 2) edge is stretched vs resting length,
    // 3) nearby projected cells cover enough of that edge.
    ...
    for (int i = 0; i < (int)boundary_membrane_pts.size(); ++i) {
        ...
        double rest_length = initial_edge_length[i];
        if (rest_length <= 1e-16) continue;

        double stretch_ratio = dist / rest_length;
        if (stretch_ratio < remesh_stretch_factor) continue;
        ...
        for (Cell* pCell : *all_cells) {
            ...
            auto projection = project_point_onto_boundary(cell_x, cell_y);
            double best_k_d = std::get<3>(projection);
            double best_t = std::get<4>(projection);
            int best_k = static_cast<int>(std::round(best_k_d));
            if (best_k != i) continue;

            // Convert projected cell width to an interval on this edge.
            double t_clamped = std::max(0.0, std::min(best_t, 1.0));
        ...
        if (supporting_cells < remesh_min_cell_matches) continue;
